...
Q_r = (kb(1)*s(1)   - kf(1)*s(7)*vac +...
       kb(3)*s(3)^2 - kf(3)*s(8)*vac^2 +...
       kb(7)*s(4)   - kf(7)*s(9)*vac)/sum(s(7:9))*V;
...
ds    = zeros(11,1);
ds(1) = kf(1)*s(7)*vac         - kb(1)*s(1) + ...
        kb(2)*s(2)^2             - kf(2)*s(1)*vac;             % dN2*/dt
ds(2) = 2*kf(2)*s(1)*vac       - 2*kb(2)*s(2)^2 + ...
        kf(6)*s(6)*vac         - kb(6)*s(2)*s(3);              % dN*/dt
ds(3) = 2*kf(3)*s(8)*vac^2     - 2*kb(3)*s(3)^2 +...
        kf(4)*s(4)*vac         - kb(4)*s(5)*s(3) +...
        kf(5)*s(5)*vac         - kb(5)*s(6)*s(3) +...
        kf(6)*s(6)*vac         - kb(6)*s(2)*s(3);              % dH*/dt
ds(4) = kb(4)*s(5)*s(3)          - kf(4)*s(4)*vac +...
        kf(7)*s(9)*vac         - kb(7)*s(4);                   % dNH3*/dt
ds(5) = kf(4)*s(4)*vac         - kb(4)*s(5)*s(3) +...
        kb(5)*s(6)*s(3)          - kf(5)*s(5)*vac;             % dNH2*/dt
ds(6) = kf(5)*s(5)*vac         - kb(5)*s(6)*s(3) +...
        kb(6)*s(2)*s(3)          - kf(6)*s(6)*vac;             % dNH*/dt
ds(7) = kb(1)*s(1)               - kf(1)*s(7)*vac +...
        Q_in/V*c_N2 - Q_out/V*s(7);                              % dN2/dt
ds(8) = kb(3)*s(3)^2             - kf(3)*s(8)*vac^2 +...
        Q_in/V*c_H2 - Q_out/V*s(8);                              % dH2/dt
ds(9) = kb(7)*s(4)               - kf(7)*s(9)*vac +...
        Q_in/V*c_NH3 - Q_out/V*s(9);                             % dNH3/dt
% dvac= kb(1)*s(1)               - kf(1)*s(7)*vac +...
%         kb(2)*s(2)^2             - kf(2)*s(1)*vac +...
%         2*kb(3)*s(3)^2           - 2*kf(3)*s(8)*vac^2 +...
%         kb(4)*s(5)*s(3)          - kf(4)*s(4)*vac +...
%         kb(5)*s(6)*s(3)          - kf(5)*s(5)*vac +...
%         kb(6)*s(2)*s(3)          - kf(6)*s(6)*vac +...
%         kb(7)*s(4)               - kf(7)*s(9)*vac;             % d*/dt
ds(10)= 0;%(q + (HORT(1:6)*R_e*T)*ds(1:6)*V + ...
...
ds(11)= 0;%(Q_in*R_e*T_orig*(c_N2*HORT_feed(7) + c_H2*HORT_feed(8) + c_NH3*HORT_feed(9))-...
%          Q_in*R_e*T*(c_N2*HORT(7) + c_H2*HORT(8) + c_NH3*HORT(9))+...
%          Q_out*R_e*T*(s(7)*HORT(7)    + s(8)*HORT(8) + s(9)*HORT(9)) -...
