...
%
global kf kb T_orig T_pulse V Q_in c_N2 c_H2 c_NH3 Isobaric Moles_SiO2_Heated...
       R_e Cp_SiO2_NIST pulse abyv

%T = T_orig;
%T = sin(pulstran(t-floor(t),[0:0.01:1],'tripuls',0.01).^2*pi/2)*(T_pulse-T_orig)+T_orig;
T = s(11);
T_gas = s(12);
...
switch pulse
    ...
    q=0.003;
    case 1
    q = sin(pulstran(t-floor(t),[0:1/3:1],'tripuls',0.01).^2*pi/2)*0.2281755020530257;
end

[kf,kb,HORT,CpOR]=amm_kinetics(T,T_orig,s);  % Obtain kinetics rate constants
...
ds(11)= (q + (HORT(1:10)*R_e*T)*ds(1:10)*V + ...
...
ds(12)= (Q_in*R_e*T*(c_N2*HORT_feed(7) + c_H2*HORT_feed(8) + c_NH3*HORT_feed(9)) -...
         Q_out*R_e*T*(s(7)*HORT(7)+s(8)*HORT(8)+s(9)*HORT(9)) -...
         h_cat*abyv*V*(T_gas - T))/...
