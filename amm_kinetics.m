function [ kf,kb,HORT,CpOR ] = amm_kinetics( T,T_orig,s )
...
kf(1) = (Stick(1)/(1-MWON*Stick(1)/2))/SDTOT*((T_gas/T_ref)^beta(1))* ...
...
kf(3) = (Stick(2)/(1-MWON*Stick(2)/2))/(abyv*SDTOT^2)*((T_gas/T_ref)^beta(3))* ...
...
kf(7) = 10*(Stick(3)/(1-MWON*Stick(3)/2))/SDTOT*((T_gas/T_ref)^beta(7))* ...
