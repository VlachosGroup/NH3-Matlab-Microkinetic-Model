...
kf=zeros(7,1);
kf(1) = abyv*(Stick_N2/(1-Stick_N2/2))*((T/T_ref)^beta)* sqrt(R_k*T/(2*pi*M_N2))* exp(Ea_kf(1)/(R_e*T));
kf(2) = A * exp((-36.7 + Ea_kf(2))/(R_e*T));
kf(3) = abyv^2*(Stick_H2/(1-Stick_H2/2)/SDEN)*((T/T_ref)^beta)* sqrt(R_k*T/(2*pi*M_H2))* exp(Ea_kf(3)/(R_e*T));
kf(4) = A * exp((-21.9 + Ea_kf(4))/(R_e*T));
kf(5) = A * exp((-18.2 + Ea_kf(5))/(R_e*T));
kf(6) = A * exp((-21.4 + Ea_kf(6))/(R_e*T));
kf(7) = abyv*(Stick_NH3/(1-Stick_NH3/2))*((T/T_ref)^beta)* sqrt(R_k*T/(2*pi*M_NH3))* exp(Ea_kf(7)/(R_e*T));
