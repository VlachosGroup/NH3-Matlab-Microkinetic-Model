...
% Kinetic rate constants
global V SDEN abyv T_ref beta omega Stoic_surf Stoic_gas Stoic
...
[Ea,Zero_BE,omega] = amm_BES_LSR(T,Stoic);
k_cov = amm_coverage(s);
%k_cov =zeros(10,1);
Ea_kf = omega(8:14).*(Stoic_surf*k_cov);
kf=zeros(7,1);
kf(1) = abyv*(Stick_N2/(1-Stick_N2/2))*((T_orig/T_ref)^beta)* ...
    sqrt(R_k*T_orig/(2*pi*M_N2))* exp(Ea_kf(1)/(R_e*T_orig));               % N2(gas)   +  Ru(Step) <--> N2(Step)
kf(2) = abyv*1.16e18*((T/T_ref)^beta) * exp((-Ea(1) + Ea_kf(2))/(R_e*T));   % N2(Step)  +  Ru(Step) <--> 2N(Step)
kf(3) = abyv^2*(Stick_H2/(1-Stick_H2/2)/SDEN)*((T_orig/T_ref)^beta)* ...
    sqrt(R_k*T_orig/(2*pi*M_H2))* exp(Ea_kf(3)/(R_e*T_orig));               % H2(Step)  + 2Ru(Step) <--> 2H(Step)
...
kf(7) = abyv*(Stick_NH3/(1-Stick_NH3/2))*((T_orig/T_ref)^beta)* ...
    sqrt(R_k*T_orig/(2*pi*M_NH3))* exp(Ea_kf(7)/(R_e*T_orig));              % NH3(gas)  +  Ru(Step) <--> NH3(Step)
